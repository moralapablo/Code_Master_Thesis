...
Automatic_example= function(n_sample,p,q_original,mean_range,beta_range,error_var,scale_method,h_1,fun,q_taylor,stepmax=1e+05){
  ...
  scaled=preprocesing(data,scale_method)
  train <- scaled$train
  test <- scaled$test
  ...
  plot1<-ggplot(df.plot,aes(x=NN.prediction,y=test$Y))+geom_point()+geom_abline(slope = 1,intercept = 0,color = "red")+labs(y = "Original Y")+labs(x = "Predicted Y with NN")+ ggtitle("NN vs Y")+theme(plot.title = element_text(hjust = 0.5)) +theme_cowplot(12) 
  
  plot2<-ggplot(df.plot,aes(x=PR.prediction,y=NN.prediction))+geom_point()+geom_abline(slope = 1,intercept = 0,color = "red")+labs(y = "Predicted Y with NN")+labs(x = "Predicted Y with PR")+theme(plot.title = element_text(hjust = 0.5)) + ggtitle("PR vs NN") +theme_cowplot(12)
  ...
  names(output)<-c("train","test","g","nn","coeff","NN.prediction","PR.prediction","NN.MSE","PR.MSE","PR.prediction.train","PR.R2","PR.R2.adjusted","plot","MSE.NN.vs.PR","original_betas")
